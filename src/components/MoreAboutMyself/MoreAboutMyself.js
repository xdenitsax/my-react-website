import React from "react";
import "./MoreAboutMyself.css";
const MoreAboutMyself = () => {
  return (
    <>
      <div className='about-me-div'>
        <div className='about-myself-content'>
          <h1 className='about-myself-header'>About me</h1>
          Hello! My name is Denitsa Georgieva and I enjoy creating things that
          live on the internet. My interest in a Software Engeneer started back
          in 2019. After studying with a law in university, I ve tried to
          develop my attorney career in Bulgaria when I realized that although I
          still had a love for studying and develop myself to something new and
          really interesting for me. After dabbling with some introductory
          JavaScript courses on the internet and exploring programming through
          self-teaching, I decided to take a leap of faith and made a switch in
          careers into tech. I have developed a strong understanding of
          JavaScript (ES5 & ES6), React, Redux, HTML5, and CSS3. As a developer,
          I find a lot of joy and excitement utilizing my skills to help
          troubleshoot bugs and pushing projects forward. I enjoy learning new
          technologies and growing in a field that is constantly being pushed to
          new boundaries. Feel free to connect with me! Let's talk about the web
          dev, tech, sports, karate, cooking, or how I can be of help to your
          team or business! I'd love to get to know more developers and expand
          my network.
        </div>
        <img src='images/Denitsa.png' alt='Denitsa' className='Denitsa-photo' />
      </div>
    </>
  );
};

export default MoreAboutMyself;
